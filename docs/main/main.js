 ...
 * control:Control move more easily
...
var Config = {
    ...
    skills: ["power", "energy", "control", "attract", "repulse", "slow"],
    attract: {Dot: 1, Power: 0.1},
    repulse: {Enemy: 1},
    coinsScale: {"1": 0.70, "5": 0.80, "10": 0.90, "20": 0.95, "50": 1, "100": 1.1, "500": 1.2, "1000": 1.3}
};
var DEBUG = 0;
...
function Game() {
    ...
    var randomPower = new Randomize().Spacing(function () {
        return Stage.Math.random(30, 150);
    });
    ...
    randomDot.Add(function (x) { //Ramp
        var n = Stage.Math.random(20, 30) | 0;
    ...
    randomDot.Add(function (x) { //Serrations
        var unitsNum = Stage.Math.random(3, 5) | 0;
        var unitLength = Stage.Math.random(5, 12) | 0;
        ...
        return game.distance < 2000 ? 0 : 1;
    });
    randomDot.Add(function (x) { //Sine Wave
        var n = Stage.Math.random(30, 50);
    ...
    randomDot.Add(function (x) { //Sine Wave XY
        ...
        return game.distance < 8000 ? 0 : game.distance < 15000 ? 1 : 2;
    });
    ...
    randomDot.Add(function (x) { //Rectangle
        ...
        return game.distance < 12000 ? 0 : 1;
    });
    ...
    randomDot.Add(function (x) { //Spray
        ...
        return game.distance < 15000 ? 0 : game.distance < 25000 ? 1 : 2;
    });
    ...
    function Player() {
        ...
        this.Attributions = function (skills) {
            ...
            this.speed = 0.2 + skills.control * 0.02;
...
Stage(function (stage) {
    ...
    (function () {
        ...
        var page_1 = Stage.image('page_1').appendTo(menu).pin('alignX', 0.5)
    ...
    (function () {
        ...
        Stage.image('energy_hint').stretch().appendTo(play).pin({
            alignX: 0,
            alignY: 0,
            offsetX: 1,
            offsetY: 1
        ...
        Stage.image('power_hint').stretch().appendTo(play).pin({
            alignX: 0,
            alignY: 0,
            offsetX: 1,
            offsetY: 8
        ...
        Stage.image('score_hint').appendTo(play).pin({
            alignX: 0,
            alignY: 0,
            offsetX: 1,
            offsetY: 15
        ...
        var uiPower = Stage.image('powerbar').stretch().appendTo(play).pin({
            alignX: 0,
            alignY: 0,
            offsetX: 20,
            offsetY: 8
        ...
        let pauseFunc = function(e){
            if(!game.pauseSpeed){
                game.pauseSpeed=game.speed;
                game.speed=0;
                play.tween(50).pin('alpha', 0.8);
            }
            else{
                play.tween(50).pin('alpha', 1);
        ...
        play.on('open', function () {
            game.Start();
            window.addEventListener('keydown', pauseFunc);
        ...
        play.on('close', function () {
            game.End();
            window.removeEventListener('keydown', pauseFunc);
        ...
        game.UIEnergy = function (e) {
            if(e<=0.1){
                uiEnergy.pin({alpha : 10* Math.max(e,0), textureAlpha :  10*Math.max(e,0)});
            }
            else{
                uiEnergy.pin({alpha : 1, textureAlpha : 1});
        ...
        game.UIPower = function (e) {
            if(e<=80){
                uiPower.pin({alpha : 0.0125*Math.max(e,0), textureAlpha : 0.0125*Math.max(e,0)});
            }
            else{
                uiPower.pin({alpha : 1, textureAlpha : 1});
